...
public class NNCFinder{
	 ...
	 */
	public NNCFinder(final DistanceMatrix distances, final int n, ArrayList<Double> demands){
	 ...
	 */
	public int findNN(int i, boolean[] nodeStatus, int k, double remQ) {
		...
		for(int j=0; j<neighbors[i].length;j++){
			if(!nodeStatus[neighbors[i][j]]){
				...
				if(demands.get(neighbor-1) < remQ) {
					counter++;
					candidate = neighbor;
					if(counter==k) {
						return neighbor;	
		...
		/**
		// Last chance: The kth node was not feasible, but maybe there's still a node we can add
		for(int j=0; j<neighbors[i].length;j++){
			if(!nodeStatus[neighbors[i][j]]){
				neighbor=neighbors[i][j];
				if(demands.get(neighbor-1) < remQ) {
	 ...
	 * returns -1;
	 */
	public int findNN(int i, boolean[] nodeStatus, double remQ) {
		return this.findNN(i, nodeStatus, 1, remQ);
	}
